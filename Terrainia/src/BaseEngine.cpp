...
void BaseEngine::CreateObjectArray(int iNumberObjects)
{
	...
	memset(&m_ppDisplayableObjects[0], 0, sizeof(DisplayableObject*) * sizeof(m_ppDisplayableObjects[0]));
}
...
void BaseEngine::DestroyOldObjects()
{
	...
	if (!m_ppDisplayableObjects.empty())
	{
		...
		m_ppDisplayableObjects.resize(0);
	}
}
...
void BaseEngine::GameRender(void)
{
	...
	if ( m_bWholeScreenUpdate )
	{
		...
		this->CopyBackgroundPixels( 1, 0, 1499, 800 );
...
void BaseEngine::AddObjectToArray(DisplayableObject* pObject)
{
	//Check if the array is empty, if not set the last NULL value to pObject
	//If it is empty then add pObject, then add NULL to the end
	if (!m_ppDisplayableObjects.empty())
	{
		m_ppDisplayableObjects.back() = pObject;
	}
	else{
